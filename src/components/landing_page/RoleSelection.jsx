...
export default function RoleSelection(){
  ...
  const handleNext = () => {
    // navigate to centralized signup page and forward the selected role via location state
  ...
  useEffect(() => {
    const interval = setInterval(() => setCurrentImg((p) => (p + 1) % images.length), 2500)
          ...
          <div className="carrier-signup-form">
            <div className="carrier-signup-field">
              ...
              <div className="role-options">
                <label className={"role-option" + (role==='carrier'? ' selected':'')}>
                  <input type="radio" name="role" value="carrier" checked={role==='carrier'} onChange={() => setRole('carrier')} />
                ...
                <label className={"role-option" + (role==='shipper'? ' selected':'')}>
            ...
            <div className="carrier-signup-login-actions">
              <button type="button" className="carrier-signup-btn" onClick={handleNext}>Next</button>
            </div>
          </div>
        </div>
      </div>
