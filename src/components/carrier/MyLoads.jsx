  ...
  const fetchLoads = async () => {
    ...
    try {
      ...
      data.loads.forEach(load => {
        ...
        // Check if load is in draft
        if (load.status === 'draft') {
          column = 'draft';
          statusFlag = 'draft';
        }
        // Check for delivered loads first (regardless of driver assignment)
        ...
        // Check if load has driver assigned
        else if (load.assigned_driver || load.assigned_driver_id) {
          if (load.driver_assignment_status === 'accepted' || load.status === 'covered') {
            statusFlag = 'accepted';
            column = 'accepted';
          } else {
            statusFlag = 'assigned'; // Driver assigned but not yet accepted
            column = 'tendered';
          }
        } else {
          // No driver assigned
